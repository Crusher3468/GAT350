...
{
	// AMBIENT
	vec3 ambient = lights[0].ambient * material.color;

	// DIFFUSE
	// calaculate light direction (unit vector)
	vec3 light_dir = normalize(vec3(lights[0].position) - position);

	// calculate light intensity with dot product (normal * light direction)
	float intensity = max(dot(light_dir, normal), 0);
	// calculate diffuse color
	vec3 diffuse = lights[0].color * material.color * intensity;
	...
	if (intensity > 0)
	{
		...
		specular = lights[0].color * material.color * intensity;
	}
